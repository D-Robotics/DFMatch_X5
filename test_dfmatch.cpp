...
int main() {
  ...
  for (int i = 0; i < 57; i++) {
    ...
    cv::Mat bgr_mat_2_resize;
    cv::resize(bgr_mat_2, bgr_mat_2_resize, cv::Size(640, 640));
    ...
    infer_x5_dfeat(bgr_mat_2_resize, dfeat_result_2);

    vector<cv::Point2f> match_pt_1, match_pt_2;

    infer_x5_lg(dfeat_result_1.first, dfeat_result_1.second, dfeat_result_2.first, dfeat_result_2.second, match_pt_1, match_pt_2);

    for (int j = 0; j < match_pt_1.size(); j++) {
      cv::Point point_img1;
      cv::Point point_img2;
      ...
      std::string match_count_text = "Matches: " + std::to_string(match_pt_1.size());
